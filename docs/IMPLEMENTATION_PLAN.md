...
| 步骤 | 任务 | 产出 |
|------|------|------|
| P0-1 | 创建项目根目录结构（backend/、frontend/、docs/、data/） | 目录骨架 |
...
| P0-8 | 下载 Solomon 算例文件（C101/C201/R101/R201/RC101/RC201）放入 `data/solomon/` | 测试数据就绪 |
| P0-9 | 下载首尔 ACVRP 数据集放入 `data/seoul/` | 仿真数据就绪 |
...
| P2-4 | 实现 `Ant` 类：单只蚂蚁构建完整解（双信息素+双启发式状态转移概率、容量约束、时间窗可行性检查、硬时间窗等待逻辑） | 蚂蚁可构建路线 |
| P2-5 | 实现改进蚁群算法 `improved_aco.py`：蚂蚁群迭代、精英蚂蚁全局信息素更新（独立 ρ_cost/ρ_time 挥发）、早停机制（patience + threshold） | 改进 ACO 可运行 |
| P2-6 | 实现 2-opt 局部搜索 `local_search.py`：路线内节点交换优化 | 解质量提升 |
| P2-7 | 将 2-opt 集成到改进 ACO 的每轮迭代最优解中 | ACO + 2-opt 完整 |
| P2-8 | 实现标准蚁群算法 `standard_aco.py`：单信息素 + 单启发式，作为对照组 | 对照算法 1 |
| P2-9 | 实现遗传算法 `genetic.py`：染色体编码（客户排列）、顺序交叉、变异、轮盘赌选择 | 对照算法 2 |
| P2-10 | 实现模拟退火 `simulated_annealing.py`：初始解（贪心）、邻域操作（swap/insert/reverse）、降温策略 | 对照算法 3 |
| P2-11 | 实现 OR-Tools 精确求解器 `ortools_solver.py`：调用 CP-SAT 或 Routing 求解小规模实例 | 锚点校验基准 |
| P2-12 | 编写算法单元测试：用 5 节点小算例验证各算法输出格式正确、Z 值合理 | 算法层质量保证 |

**依赖关系：** P2-1 → P2-2 → P2-3 → P2-4 → P2-5 → P2-6 → P2-7；P2-8/P2-9/P2-10 依赖 P2-1 和 P2-2 可并行；P2-11 独立；P2-12 在所有算法完成后
...
| P8-5 | 不可达客户预检：算法启动前检测硬时间窗不可达客户，前端 Alert 告警展示 | 预检功能可用 |
| P8-6 | 端到端冒烟测试：完整走通登录→加载 C101→改进 ACO 求解→查看看板→对比 4 算法全流程 | 系统可演示 |
| P8-7 | 编写部署说明：本地启动步骤（后端 `python run.py` + 前端 `npm run dev`），环境要求 | 部署文档就绪 |

**依赖关系：** P8-1/P8-2 可并行（Nice-to-have，可跳过）；P8-3/P8-4/P8-5 可并行；P8-6 在所有功能完成后；P8-7 最后
